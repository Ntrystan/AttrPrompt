    ...
    with open(f"../datasets/{dataset}/attrprompt/gpt-3.5-turbo/{field}/{label}.jsonl", "r") as f:
        for lines in f:
            ...
            for x in keys_label:
                if label != x:
                    if (
                        fuzz.ratio(text, x) >= 90
                        or f"{x} " in f"{text} "
                        or text in x
                    ):
                        print(text, "orig", label, "overlap", x, fuzz.ratio(text, x), text in x)
                        if fuzz.ratio(text, x) >= 90 or f"{x} " in f"{text} ":
                            remove_keyword_lst.extend((x, text))
                        elif x not in keys[text]:
                            keys[text] += [x]
...
for y in keys:
    ...
    idxs = list({id2label[z] for z in keys[y]})
